...
class RemoteExecutor(object):
    ...
    def _run(self, command, is_sudo):
        ...
        if is_sudo:
            ...
            prefix = 'sudo -p \\"{prompt}\\" '.format(prompt=sudo_prompt)
        else:
            sudo_prompt = None
            prefix = ''
        ...
        with session:
            ...
            if not self.ignore_errors and session.recv_exit_status() != 0:
                message = 'Error running command on host {host}: {cmd}'.format(
