...
class Prerequisites(generate_load.Prerequisites):
    ...
    def create_keypairs(self):
        try:
            for user, keypair in zip(self.config.users, self.config.keypairs):
                if user['enabled']:
                    self.switch_user(user=user['name'], tenant=user['tenant'],
    ...
    def create_vms(self):
        for vm in self.config.vms:
            ...
            vm['nics'] = [{'net-id': self.get_net_id(
                self.config.networks[0]['name'])}]
            self.check_vm_state(self.novaclient.servers.create(**vm))
        ext_net_id = self.get_net_id(self.config.ext_net['name'])
        port_id = None
        ...
        net_name = self.config.networks[0]['name']
        for vm in src_vms:
    ...
    def create_all_networking(self):
        ...
        self.neutronclient.add_interface_router(self.get_router_id(
            self.config.routers[0]['router']['name']), {"subnet_id": subnet})
    ...
    def clean_objects(self):
        ...
        try:
            self.neutronclient.remove_gateway_router(
                self.get_router_id(self.config.routers))
        except Exception as e:
